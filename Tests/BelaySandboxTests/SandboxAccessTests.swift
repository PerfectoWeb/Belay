...
/// in `swift test`, which is not sandboxed; `BelayAppTests` is hosted by the
...
final class SandboxAccessTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        suite = "belay.sandbox.\(UUID().uuidString)"
    ...
    func testTheTestHostIsGrantedReadsTheShippingBuildIsNot() throws {
        ...
        let shipping = try String(
            contentsOf: URL(fileURLWithPath: #filePath)
                ...
                .appendingPathComponent("Resources/Entitlements/Belay-MAS.entitlements"),
            encoding: .utf8)
        XCTAssertFalse(
    ...
    private func scratchDirectory() throws -> URL {
        let root = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
            .appendingPathComponent("belay-sandbox-\(UUID().uuidString)", isDirectory: true)
