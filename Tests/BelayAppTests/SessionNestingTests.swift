...
///
/// The panel showed two rows both titled "Belay", one Working and one Idle, with
...
final class SessionDisambiguationTests: XCTestCase {
    ...
    func testASingleSessionIsPlain() {
        let rows = SessionRow.disambiguate([row("s1", workspace: "Belay", name: "belay-9a")])
        XCTAssertNil(rows[0].detail)
        XCTAssertEqual(rows[0].title, "Belay", "a lone session grew a suffix nobody needs")
    }

    func testTwoSessionsInOneWorkspaceBothGetASuffix() {
        let rows = SessionRow.disambiguate([
            row("s1", workspace: "Belay", name: "belay-9a"),
            row("s2", workspace: "Belay", name: "belay-74")
        ])
        XCTAssertEqual(rows.map(\.detail), ["9a", "74"])
        XCTAssertEqual(rows.map(\.title), ["Belay · 9a", "Belay · 74"])
    }
    ...
    func testSessionsInDifferentWorkspacesStayPlain() {
        ...
        XCTAssertEqual(rows.map(\.title), ["Belay", "Ledger"])
    }
    ...
    func testARowWithNoNameFallsBackToTheWorkspace() {
        let rows = SessionRow.disambiguate([
            row("s1", workspace: "Belay", name: nil),
            row("s2", workspace: "Belay", name: "belay-74")
        ])
        XCTAssertEqual(rows.map(\.title), ["Belay", "Belay · 74"])
    }
