...
final class AboutAnimationTests: XCTestCase {
    ...
    func testEveryFrameIsReachable() {
        let cycle = (0..<BelayGlyph.frameCount).map(BelayGlyph.frameDuration).reduce(0, +)
        let start = Date(timeIntervalSinceReferenceDate: 0)
        var seen = Set<Int>()
        for step in 0..<400 {
            seen.insert(BelayGlyph.frame(at: start.addingTimeInterval(cycle * Double(step) / 400)))
        }
        XCTAssertEqual(seen.count, BelayGlyph.frameCount, "some frames are never shown")
    }
    ...
    func testTheHoldIsMostOfTheCycle() {
        let cycle = (0..<BelayGlyph.frameCount).map(BelayGlyph.frameDuration).reduce(0, +)
        let hold = BelayGlyph.frameDuration(BelayGlyph.frameCount - 1)
    ...
    func testFramesAdvanceInOrder() {
        let start = Date(timeIntervalSinceReferenceDate: 0)
        var last = BelayGlyph.frame(at: start)
        var wraps = 0
        for step in 1...200 {
            let frame = BelayGlyph.frame(at: start.addingTimeInterval(Double(step) * 0.05))
            if frame < last {
                wraps += 1
            } else {
