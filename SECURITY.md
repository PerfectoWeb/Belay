...
Vigil reads local agent state to decide whether an AI coding agent is working.
...
- Vigil makes **one** outbound connection, and only if you ask for it: an HTTPS
  ...
  no crash reporter. With the update check off, Vigil opens no sockets at all
...
## What Vigil reads, precisely

### Transcript files (`~/.claude/projects/**/*.jsonl`)

Vigil keeps a byte offset per file and reads only what was appended since it last
...
refinement layered on top, and if it ever fails Vigil falls back to "the file
...
conversation. Vigil never reads them from the beginning after the first sight of
a file, never re-reads history, and caps how much it will read in one go.

### Session records (`~/.claude/sessions/*.json`)

Vigil reads `pid`, `sessionId` and `cwd` to know which agent processes are alive
...
If you turn on precise detection, Claude Code POSTs lifecycle events to Vigil's
loopback listener. Those payloads include a `prompt` field containing **your full
prompt text**.

Vigil's decoder declares keys for exactly four fields — `session_id`,
...
nor any file Vigil writes.

## What Vigil writes

- `~/Library/Application Support/Vigil/bridge.json` — the loopback port and a
  random per-install token, created `0600`.
- `~/Library/Application Support/Vigil/backups/` — timestamped copies of your
...
2. If the file is not plain JSON, Vigil **refuses to write it** and offers you a
   snippet to paste instead.
3. Writes are atomic (temp file in the same directory, then `replaceItem`).
4. Every entry Vigil adds is marked as its own, so removal is exact and never
   touches a hook you added.

## The loopback listener

Only present while Vigil is running. Bound to `127.0.0.1`, never `0.0.0.0`.
...
behaviour, not something Vigil chooses, and it is the same for any loopback
...
entitlement, so it is not merely that Vigil does not phone home — it cannot.

## Verify it yourself

```bash
# What Vigil is holding, and why, in plain language:
pmset -g assertions | grep "pid $(pgrep -x Vigil)("

# What it is listening on — expect 127.0.0.1, never *:
lsof -nP -iTCP -sTCP:LISTEN -a -p "$(pgrep -x Vigil)"
...
Vigil is not a security product. It defends against accidental exposure of your
...
directly and does not need Vigil to do it.
