...
Claude Code is P0 and must be excellent. Codex CLI is P1. A generic provider
(watch a folder / watch a process name / accept a local webhook) is P1 and
covers DeepSeek, Gemini CLI, Aider, Cursor and anything future without a code
change. A new first-class provider must be addable by writing one file that
conforms to one protocol.
