...
| `btn-download-green.png` | The download button on a phone, at 64 |
| `btn-download-green-desk.png` | The same button on a wide screen, at 60. Its own drawing, not a resize |
| `btn-site-dark.png`, `btn-site-light.png` | Learn More on a phone, one file per GitHub theme |
...
period when the `-desk` button variants were produced by padding the plain ones
programmatically, and it went wrong in the way that arrangement always does: the
originals were redrawn, the derived copies were not, and a wide screen kept
showing an older design that nobody had chosen. If a size or a variant is
needed, it is drawn, not computed.
