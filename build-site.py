...
def head(code, title, meta, depth, page):
    ...
    # Three empty spans and no text: this is light, so it says nothing to a
    # screen reader and answers no clicks.
    lines += [
        "</head>",
        "<body>",
        '<div class="lights" aria-hidden="true"><i></i><i></i><i></i></div>',
        "<script>",
        "  // Three lights, each one appearing somewhere else every time.",
        "  //",
        "  // A stylesheet can move a light but it cannot surprise you with one: the",
        "  // place, the size, the colour, the direction and the lifetime all have to",
        "  // be picked again for every appearance, and picking is what CSS does not",
        "  // do. So each light lives a lap here, fade in, drift, fade out, and when",
        "  // the lap finishes it starts another one from new numbers.",
        "  (function () {",
        "    var box = document.querySelector('.lights');",
        "    if (!box || !box.animate) { return; }",
        "    var lights = Array.prototype.slice.call(box.querySelectorAll('i'));",
        "    var still = window.matchMedia('(prefers-reduced-motion: reduce)').matches;",
        "",
        "    function unit() { return Math.max(window.innerWidth, window.innerHeight) / 100; }",
        "    function pick(low, high) { return low + Math.random() * (high - low); }",
        "",
        "    // The headline is the one place a light may not sit behind. Its unfilled",
        "    // grey is tuned a couple of points above the page, and anything brighter",
        "    // underneath swallows it.",
        "    //",
        "    // Only the vertical distance is tested. The headline runs the width of the",
        "    // column, so a light beside it is a light on it, and a band is a test that",
        "    // can always be satisfied by looking further up or down. Read live rather",
        "    // than once: scroll far enough and the headline is off screen, at which",
        "    // point there is nothing to avoid.",
        "    function band(room) {",
        "      var head = document.querySelector('h1');",
        "      if (!head) { return null; }",
        "      var r = head.getBoundingClientRect();",
        "      if (r.bottom < 0 || r.top > window.innerHeight) { return null; }",
        "      return { top: r.top - room, bottom: r.bottom + room };",
        "    }",
        "",
        "    function lap(el) {",
        "      var u = unit();",
        "      var w = pick(36, 56) * u;",
        "      var h = w * pick(0.7, 0.95);",
        "      var keep = band(h * 0.22);",
        "      var x = pick(-0.15, 1.15) * window.innerWidth;",
        "      var y = 0;",
        "      for (var tries = 0; tries < 20; tries++) {",
        "        y = pick(-0.15, 1.15) * window.innerHeight;",
        "        if (!keep || y < keep.top || y > keep.bottom) { break; }",
        "      }",
        "      // A window short enough that the band fills it leaves no room to roll",
        "      // into, so take the larger side rather than giving up and landing on",
        "      // the words.",
        "      if (keep && y >= keep.top && y <= keep.bottom) {",
        "        y = keep.top > window.innerHeight - keep.bottom",
        "          ? pick(-0.15, 1) * keep.top",
        "          : keep.bottom + pick(0, 1) * (window.innerHeight - keep.bottom);",
        "      }",
        "      el.style.width = Math.round(w) + 'px';",
        "      el.style.height = Math.round(h) + 'px';",
        "      el.style.left = Math.round(x - w / 2) + 'px';",
        "      el.style.top = Math.round(y - h / 2) + 'px';",
        "      // Dealt rather than rolled. A coin per appearance came up the same",
        "      // three times often enough to look deliberate \u2014 with three lights and",
        "      // a fair coin it is one round in four \u2014 and three of one colour reads",
        "      // as a single wash instead of as separate lights. Each light simply",
        "      // takes the other tone from the one it had, and the three start on",
        "      // opposite feet, so there is always a mix on screen.",
        "      el.dataset.tone = el.dataset.tone === 'violet' ? 'blue' : 'violet';",
        "      el.style.setProperty('--tone', 'var(--glow-' + el.dataset.tone + ')');",
        "      // Not every light at full strength, or three of them at once reads as",
        "      // one even wash.",
        "      el.style.setProperty('--peak',",
        "        'calc(var(--glow-strength) * ' + pick(0.7, 1).toFixed(2) + ')');",
        "",
        "      if (still) { el.style.opacity = '0.7'; return; }",
        "",
        "      var away = pick(16, 30) * u;",
        "      var turn = pick(0, Math.PI * 2);",
        "      var life = pick(9, 15) * 1000;",
        "      var run = el.animate([",
        "        { transform: 'translate3d(0,0,0) scale(0.88)', opacity: 0 },",
        "        // Up quickly, held for most of the lap, down quickly. Spending a",
        "        // third of every lap fading each way left all three faint at once",
        "        // and the page looking as though it had nothing behind it.",
        "        { opacity: 1, offset: 0.2 },",
        "        { opacity: 1, offset: 0.8 },",
        "        { transform: 'translate3d(' + (Math.cos(turn) * away).toFixed(1) + 'px,'",
        "          + (Math.sin(turn) * away).toFixed(1) + 'px,0) scale(1.14)', opacity: 0 }",
        "      ], { duration: life, easing: 'ease-in-out' });",
        "      run.finished.then(function () { lap(el); }).catch(function () {});",
        "    }",
        "",
        "    // The page opens dark and the room fills in: a second or two of nothing,",
        "    // then one light, then the next. Starting all three lit was tried and",
        "    // reads as a background rather than as something happening, and the",
        "    // arrival is the best part of it.",
        "    //",
        "    // Reduce Motion gets its light at once, because there is no arrival to",
        "    // wait for when nothing is going to move.",
        "    lights.forEach(function (el, i) {",
        "      // Odd ones start blue, even ones violet: `lap` flips before it reads,",
        "      // so this is the tone *before* the first appearance.",
        "      el.dataset.tone = i % 2 ? 'blue' : 'violet';",
        "      setTimeout(function () { lap(el); },",
        "        still ? 0 : pick(0.8, 2.6) * 1000 + i * 1600);",
        "    });",
        "  })();",
        "</script>",
        '<div class="wrap">',
...
def landing(code):
    ...
    lines += [
        ...
        "        await wait(Math.max(0, going - 1) * 180 + 800);",
        "        await wait(900);",
        "      }",
        "    }",
        "    cycle();",
