...
def head(code, title, meta, depth, page):
    ...
    lines += [
        ...
        "      el.style.setProperty('--tone',",
        "        Math.random() < 0.6 ? 'var(--glow-blue)' : 'var(--glow-violet)');",
        ...
        "      // The first lap starts in the middle of itself, so the lights are",
        "      // already lit in the first frame the reader sees. Starting all three",
        "      // from zero meant several seconds of a page with nothing behind it,",
        "      // and the three of them blooming together afterwards.",
        "      if (opening) { run.currentTime = life * pick(0.25, 0.6); }",
        "      run.finished.then(function () { lap(el, false); }).catch(function () {});",
        "    }",
        "",
        "    lights.forEach(function (el) { lap(el, true); });",
        "  })();",
        "</script>",
        '<div class="wrap">',
        "",
