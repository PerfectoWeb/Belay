...
def head(code, title, meta, depth, page):
    ...
    lines += [
        ...
        "    // underneath swallows it, so a rolled position that lands on the words is",
        "    // rolled again. Read live rather than once: scroll far enough and the",
        "    // headline is off screen, at which point there is nothing to avoid.",
        "    function clear(x, y, room) {",
        "      var head = document.querySelector('h1');",
        "      if (!head) { return true; }",
        "      var r = head.getBoundingClientRect();",
        "      if (r.bottom < 0 || r.top > window.innerHeight) { return true; }",
        "      return x < r.left - room || x > r.right + room",
        "        || y < r.top - room || y > r.bottom + room;",
        "    }",
        "",
        "    function lap(el) {",
        "      var u = unit();",
        "      // Wide enough to read as a wash rather than as a ball, and nothing",
        "      // like the window: a light the size of the page is a background, not a",
        "      // light.",
        "      var w = pick(20, 34) * u;",
        "      var h = w * pick(0.7, 0.95);",
        "      var x = 0, y = 0;",
        "      for (var tries = 0; tries < 30; tries++) {",
        "        x = pick(-0.1, 1.1) * window.innerWidth;",
        "        y = pick(-0.1, 1.1) * window.innerHeight;",
        "        if (clear(x, y, w * 0.3)) { break; }",
        "      }",
        ...
        "        'calc(var(--glow-strength) * ' + pick(0.65, 1).toFixed(2) + ')');",
        "",
        "      if (still) { el.style.opacity = '0.7'; return; }",
        "",
        "      var away = pick(7, 15) * u;",
        "      var turn = pick(0, Math.PI * 2);",
        "      var life = pick(15, 26) * 1000;",
        "      var run = el.animate([",
        ...
        "    }",
        "",
        "    // Staggered, so the three of them are never in the same part of their",
        "    // lap and the group never blooms or empties all at once.",
        "    lights.forEach(function (el, i) {",
        "      setTimeout(function () { lap(el); }, still ? 0 : i * 4200 + Math.random() * 2000);",
        "    });",
        "  })();",
        "</script>",
        '<div class="wrap">',
