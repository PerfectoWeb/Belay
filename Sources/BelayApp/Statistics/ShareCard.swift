...
/// The picture Belay hands to a share sheet: 1200x630, dark, always identical.
///
...
/// often than not.
///
/// The lockup is `BelayWordmark`, the same one About shows, rather than a glyph
/// and a `Text` set next to each other here. This is the one image of Belay most
...
struct ShareCard: View {
    ...
    private var header: some View {
        HStack(alignment: .firstTextBaseline, spacing: 24) {
            BelayWordmark(size: 44, word: Ink.text, mark: Ink.accent)
            Spacer(minLength: 24)
