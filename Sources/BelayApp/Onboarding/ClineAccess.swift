...
enum ClineAccess {
    ...
    static var folder: URL { home.appendingPathComponent(".cline", isDirectory: true) }

    #if BELAY_MAS

    private static let bookmarked = BookmarkFileAccess.clineHome(home: home)
    ...
    static func request() -> Bool {
        let picked = ClaudeFolderPanel.run(
            ...
            prompt: String(localized: "Grant access to ~/.cline"))
        guard let picked else { return isGranted }
        do {
            try bookmarked.grant(picked)
        } catch {
            Log.app.error("could not bookmark the cline folder: \(error, privacy: .public)")
        }
        return isGranted
    }
