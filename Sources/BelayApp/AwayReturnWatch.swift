...
struct AwaySummary: Equatable {
    ...
    mutating func tick(idle: TimeInterval, holding: Bool, now: Date) -> Report? {
        defer { lastTick = now }
        let isAway = AwayTime.isAway(idle)

        if away, !isAway {
            // The return. Say it or drop it, then start clean either way.
            let report = Report(heldAway: heldAway, finishedRuns: finishedRuns)
            away = false
            heldAway = 0
            finishedRuns = 0
...
final class AwayReturnWatch {
    ...
    private func tick(now: Date) {
        ...
        if let report = summary.tick(idle: idle, holding: holding, now: now) {
            Diagnostics.note(
