...
struct HookPreviewSheet: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            if let preview = precise.preview() {
                Text("Vigil will change \(precise.settingsPath) to:")
                    .font(.callout)
    ...
    private var unsafeToEdit: some View {
        Text(
            """
            Vigil will not edit this file, because it is not plain JSON. It may \
            have comments or a format Vigil cannot write back safely. Add this \
            yourself instead:
            """
        )
        .font(.callout)
    ...
    private func codeBlock(_ text: String) -> some View {
        ...
        .accessibilityLabel("The exact configuration Vigil will add")
    }
}
