...
///
/// It lives in the app layer rather than in `VigilSettings` because it is a
/// provider's own configuration, and `VigilSettings` models Vigil's policy — the
...
struct GenericTargetStore {
    ...
    init(defaults: UserDefaults = UserDefaults(suiteName: "com.perfecto-web.vigil") ?? .standard) {
