...
///
/// Split out of `VigilController` only to keep that file under the length rule.
extension VigilController {
    /// `~/.claude/projects` does not exist until Claude Code opens its first
    /// project, and a new user installs Vigil before that happens. Without this
