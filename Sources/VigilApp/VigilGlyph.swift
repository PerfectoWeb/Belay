...
/// Vigil's mark: a large sparkle with two smaller ones that twinkle while an
/// agent is working.
///
...
@MainActor
enum VigilGlyph {
    /// What the mark shows. Fewer looks than there are `VigilState` cases on
    ...
    enum Look: Hashable {
        ...
        case calling
        /// Vigil is holding nothing: all three, dim.
        case off
    ...
    /// happened: Vigil is running, watching, and has deliberately let go to save
