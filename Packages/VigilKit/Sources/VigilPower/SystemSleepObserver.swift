...
/// nothing but `VigilApp` imports AppKit. The AppKit-free alternative,
/// `IORegisterForSystemPower`, would oblige VigilPower to own a run loop and to
...
public actor SystemSleepObserver {
    ...
    /// Called by `VigilApp` from its `NSWorkspace` observers.
    ///
