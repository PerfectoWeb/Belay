...
`.inferred`; `.exact` comes from `VigilHookBridge`.
...
**Depends on:** `VigilSupport` and `VigilCore`. Filesystem access goes through
...
ours to read. A transcript that appears *while* Vigil is running is news, so that
