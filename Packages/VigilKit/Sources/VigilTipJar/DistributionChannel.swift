...
///
/// The app target's `VIGIL_MAS` compile condition does not reach a local
/// SwiftPM target: Xcode builds package targets from the package's own
/// settings, so `#if VIGIL_MAS` inside VigilKit is false in every build
...
/// `#if VIGIL_MAS` stays in the app target where it actually means something.
public enum DistributionChannel: String, Sendable {
    case direct
    case appStore

    public static let infoKey = "VigilDistributionChannel"
