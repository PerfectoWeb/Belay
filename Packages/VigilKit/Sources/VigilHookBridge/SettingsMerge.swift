...
/// Pure merge of Vigil's hooks into a parsed `settings.json`, and back out again.
///
...
/// The rule throughout is that anything Vigil does not recognise is copied
...
enum SettingsMerge {
    ...
    static func installedURLs(in settings: [String: Any]) -> [String] {
        ...
        return section.values
            ...
            .flatMap { $0 }
            .compactMap(HookConfiguration.vigilURL)
    }
    ...
    /// `nil` means the whole group was Vigil's and should disappear.
    private static func strip(group: Any, removedAny: inout Bool) -> Any? {
        ...
        let kept = entries.filter { !HookConfiguration.isVigilEntry($0) }
