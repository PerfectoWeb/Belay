...
a Vigil value, stored, or handed to a logger. It is not `public` precisely so that
...
front of the user's agent. `docs/00-INVARIANTS.md` invariant 5 says Vigil does not get to do
...
**The ownership marker is a query item in Vigil's own hook URL, not a JSON key.**
`http://127.0.0.1:<port>/hook?src=vigil`. Two reasons. Claude Code validates the
...
hooks on the same events — get tested exactly. Anything Vigil does not recognise
...
changed ephemeral port, and it deliberately does nothing when Vigil is not already
