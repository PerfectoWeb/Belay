...
renders. `AwakePolicy` declares every tunable, with the defaults that make Vigil
behave correctly for someone who never opens Settings.

**Depends on:** `VigilSupport`, and nothing else — not IOKit, not the filesystem,
...
cooling-down unchanged; only `VigilState` moves. Swapping the reason the moment a
...
stale can pin the Mac awake. Recorded as D9 in `PROJECT_STATE.md`.
