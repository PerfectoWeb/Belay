...
/// Vigil's preferences, typed.
///
/// Reads are cheap (the plist is read once at init and clamped); writes persist
/// immediately and notify observers. `policy` is the only thing `VigilCore`
...
public final class SettingsStore {
    ...
    public static let suiteName = "com.perfecto-web.vigil"
    ...
    public init(defaults: UserDefaults) {
        ...
        migration = outcome
        // A store written by a newer Vigil is unreadable by definition, so we run
    ...
    /// The one value type `VigilCore` consumes. Always in range.
