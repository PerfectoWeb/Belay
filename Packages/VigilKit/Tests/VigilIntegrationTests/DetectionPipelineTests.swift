...
struct DetectionPipelineTests {
    ...
    struct Pipeline {
        ...
        /// severs provider → coordinator. `VigilController` retains it as a
    ...
    private func makePipeline(projects: URL, sessions: URL) async -> Pipeline {
        ...
        // would never expire. The real app starts one in VigilController.
    ...
    private func makeWorkspace() throws -> Workspace {
        let root = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("vigil-pipeline-\(UUID().uuidString)", isDirectory: true)
