...
struct HookEnvelopeTests {
    ...
    /// user's prompt and the rest of Vigil. Read the header of HookEnvelope.swift
    ...
    @Test func recognisesOnlyItsOwnEntries() {
        ...
        #expect(HookConfiguration.isVigilEntry(ours))
        #expect(HookConfiguration.vigilURL(ours) == BridgeScratch.endpoint.url)

        let lookalikes: [[String: Any]] = [
            ["type": "http", "url": "http://127.0.0.1:51234/hook"],
            ["type": "command", "command": "curl http://127.0.0.1:51234/hook?src=vigil"],
            ["type": "http", "url": "http://127.0.0.1:51234/other?src=vigil"],
            ["type": "http", "url": "http://127.0.0.1:51234/hook?src=other"]
        ]
        for entry in lookalikes {
            #expect(HookConfiguration.isVigilEntry(entry) == false, "\(entry)")
        }
    }
}
...
struct HookRequestParserTests {
    private func head(length: Int, token: String = "abc") -> String {
        """
        POST /hook?src=vigil HTTP/1.1\r
        Host: 127.0.0.1\r
    ...
    @Test func waitsForTheWholeBody() {
        ...
        #expect(request.path == "/hook?src=vigil")
