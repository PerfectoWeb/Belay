...
Depends on `BelaySupport` only. No AppKit: `LinkTipJar` takes an injected
...
**The channel is chosen at runtime, not with `#if BELAY_MAS`.**
...
set on an app target **does not reach a local SwiftPM target**. `#if BELAY_MAS`
...
`UpdateChannel` lives here rather than in a `BelayUpdates` target of its own for
