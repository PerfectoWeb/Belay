...
Belay's preferences, typed and clamped, plus the schema versioning that lets them
change later.
...
lives in `BelayCore`; this module builds one from what is on disk. So the
...
**A store written by a newer Belay is ignored, not guessed at.**
