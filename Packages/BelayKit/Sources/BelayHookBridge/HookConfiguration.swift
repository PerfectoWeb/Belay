...
/// The JSON Belay writes into `~/.claude/settings.json`, and the rule it uses to
...
enum HookConfiguration {
    ...
    static func entry(for endpoint: BridgeEndpoint) -> [String: Any] {
        [
            ...
            // The one line that makes it impossible for Belay to slow the user's
    ...
    /// Belay's ownership marker lives in the hook's `url` as a query item rather
    ...
    static func isBelayEntry(_ value: Any) -> Bool {
        guard let url = belayURL(value) else { return false }
        return !url.isEmpty
    }

    /// The URL of a Belay-owned entry, or `nil` if the entry is not ours. This
