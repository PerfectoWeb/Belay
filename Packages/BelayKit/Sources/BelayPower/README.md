...
**Depends on:** `BelaySupport`. It has never heard of a session and does not
import `BelayCore`.
...
rather than release-and-recreate. If Belay crashes, hangs or is killed, the Mac
...
`NSWorkspace`'s own notification centre, and nothing but `BelayApp` may import
