...
struct HookReceiverTests {
    ...
    /// all. Nothing in Belay may ever be able to repeat what it says.
    ...
    @Test func answersAMalformedBodyWithoutEmittingAnything() async throws {
        try await withReceiver { endpoint, collector in
            for body in ["not json at all", "", "{\"hook_event_name\":\"Stop\"}", "[]"] {
                ...
                #expect(status == 204, "a body Belay cannot use is still a body it must answer")
            }
    ...
    /// produces a signal, and the prompt appears in nothing Belay keeps.
    ...
    @discardableResult
    private func post(
        _ body: String, to endpoint: BridgeEndpoint, token: String?, path: String = "/hook?src=belay"
    ) async throws -> Int {
