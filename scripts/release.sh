...
# login keychain (docs/BLOCKERS.md (git history) B1) and notarization is closed (B6).
#
...
# ---------------------------------------------------------------- placeholders
# PLACEHOLDER — docs/BLOCKERS.md (git history) B1. Set BELAY_TEAM_ID in the environment or replace
...
[ "$TEAM_ID" != "ABCDE12345" ] || die "BELAY_TEAM_ID is still the placeholder. See docs/BLOCKERS.md (git history) B1."
...
if [ "$SKIP_NOTARIZE" -eq 0 ]; then
  ...
  or put the key and ids in .secrets/appstoreconnect.env. See docs/BLOCKERS.md (git history) B6."
    fi
fi
